...
public class RestaurantManagementPanel extends JPanel {
	...
	private void initComponents() {
		...
		headerPanel.setBackground(PRIMARY_BLUE); // WindowBuilder nhận màu nền
		var lblTitle = new JLabel("QUẢN LÝ NHÀ HÀNG", SwingConstants.CENTER);
		...
		txtSearch = new JTextField("Tìm theo tên nhà hàng hoặc Street ID...");
	...
	private void loadData() {
		model.setRowCount(0);
		var list = new RestaurantDAO().getAll();
		for (Restaurant r : list) {
			model.addRow(new Object[]{r.getId(), r.getName(), r.getStreetId()});
		}
	}
	...
	private void searchRestaurant() {
		...
		for (Restaurant r : list) {
			if (r.getName().toLowerCase().contains(key)
					|| String.valueOf(r.getStreetId()).contains(key)) {
				model.addRow(new Object[]{r.getId(), r.getName(), r.getStreetId()});
			}
		}
	}
	...
	private void deleteRow() {
		...
		if (row == -1) {
			JOptionPane.showMessageDialog(this, "Vui lòng chọn nhà hàng cần xóa!", "Cảnh báo", JOptionPane.WARNING_MESSAGE);
			return;
		}
		var id = (int) model.getValueAt(row, 0);
		if (JOptionPane.showConfirmDialog(this, "Bạn có chắc muốn xóa nhà hàng ID: " + id + "?",
				"Xác nhận", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
			if (new RestaurantDAO().delete(id)) {
	...
	private void exportPDF() {
		try {
			var header = new java.text.MessageFormat("DANH SÁCH NHÀ HÀNG");
