...
public class ContractManagementPanel extends JPanel {
	...
	public ContractManagementPanel() {
		...
		var lblTitle = new JLabel("QUẢN LÝ HỢP ĐỒNG", SwingConstants.CENTER);
	...
	private JPanel createSearchPanel() {
		...
		txtSearch = new JTextField("Tìm theo vị trí, nhân viên hoặc trạng thái...");
		txtSearch.setFont(new Font("Segoe UI", Font.PLAIN, 13));
		txtSearch.setPreferredSize(new Dimension(380, 38));
		...
		btnSearch.addActionListener(e -> searchContract());
		panel.add(btnSearch);
	...
	private JPanel createTableCard() {
		...
		var lbl = new JLabel("Danh sách hợp đồng");
		lbl.setFont(new Font("Segoe UI", Font.BOLD, 15));
		lbl.setForeground(PRIMARY_BLUE);
		lbl.setBorder(new EmptyBorder(5, 10, 10, 0));
		card.add(lbl, BorderLayout.NORTH);

		String[] columns = { "ID", "Mã NV", "Vị trí", "Lương", "Bắt đầu", "Kết thúc", "Trạng thái" };
	...
	private void loadData() {
		...
		for (Contract c : list) {
			model.addRow(new Object[]{
					c.getContractId(), c.getEmployeeId(), c.getPosition(),
					c.getSalary(), c.getStartDate(), c.getEndDate(), c.getStatus()
			});
		}
	}
	...
	private void searchContract() {
		...
		for (Contract c : list) {
			if (c.getPosition().toLowerCase().contains(key.toLowerCase())
					|| c.getStatus().toLowerCase().contains(key.toLowerCase())
					|| String.valueOf(c.getEmployeeId()).contains(key)) {
				model.addRow(new Object[]{
						c.getContractId(), c.getEmployeeId(), c.getPosition(),
						c.getSalary(), c.getStartDate(), c.getEndDate(), c.getStatus()
				});
	...
	private void deleteRow() {
		...
		if (row == -1) {
			JOptionPane.showMessageDialog(this, "Vui lòng chọn hợp đồng cần xóa!", "Cảnh báo",
					JOptionPane.WARNING_MESSAGE);
			return;
		}
		var id = (int) model.getValueAt(row, 0);
		if (JOptionPane.showConfirmDialog(this, "Bạn có chắc muốn xóa hợp đồng ID: " + id + "?", "Xác nhận",
				JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
			if (new ContractDAO().delete(id)) {
				JOptionPane.showMessageDialog(this, "Xóa thành công!");
				loadData();
			} else {
				JOptionPane.showMessageDialog(this, "Không thể xóa hợp đồng!", "Lỗi", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
	...
	private void exportPDF() {
		try {
			var header = new java.text.MessageFormat("DANH SÁCH HỢP ĐỒNG");
